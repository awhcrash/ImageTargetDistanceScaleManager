...
public class ImageTargetDistanceScaleManager : MonoBehaviour
{
    ...
    private static readonly float maxScale = 2.0f;      // helper value indicating the maximum scaling of our CONTENT AR marker's virtual object
    private static readonly float minScale = 0.5f;      // helper value indicating the minimum scaling of our CONTENT AR marker's virtual object
    ...
    private void Update()
    {
        ...
        if (imageTargetContent.isTracked)
        {
            ...
            // while CONTENT AR marker is tracked, check also if MINUS AR marker is currently tracked
            if (imageTargetMinus.isTracked)
            {
                // CONTENT and MINUS AR markers are tracked at the same time: calculate distance and scale down the CONTENT AR marker's virtual object accordingly
    ...
    private void scaleUpContent(float distance)
    {
        ...
        // check for upper scaling minimum (to restrict the virtual object from growing any further)
        ...
        {
            // if min scaling is not yet reached: apply scaling
            contentVirtualObject.transform.localScale -= new Vector3(scaleMultiplier, scaleMultiplier, scaleMultiplier);
        }
